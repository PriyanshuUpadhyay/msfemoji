import React from "react";
export function HIGH_CONTRAST_CINEMA() {
  return (
    <>
      <svg
        width="100%"
        height="100%"
        viewBox="0 0 32 32"
        fill="none"
        xmlns="http://www.w3.org/2000/svg"
      >
        <path
          d="M15.5 13C17.433 13 19 11.433 19 9.5C19 7.567 17.433 6 15.5 6C13.567 6 12 7.567 12 9.5C12 11.433 13.567 13 15.5 13Z"
          fill="#212121"
        />
        <path
          d="M12 16.5C12 15.1193 13.1193 14 14.5 14H22.5C23.8807 14 25 15.1193 25 16.5V21.5C25 22.8807 23.8807 24 22.5 24H14.5C13.1193 24 12 22.8807 12 21.5V16.5Z"
          fill="#212121"
        />
        <path
          d="M7.04688 14.8631V23.1893C7.04688 23.8575 7.85473 24.1921 8.32721 23.7197L10.796 21.2509C10.9366 21.1103 11.0156 20.9195 11.0156 20.7206V17.3124C11.0156 17.1125 10.9358 16.9208 10.7939 16.78L8.3251 14.3307C7.85178 13.8611 7.04688 14.1963 7.04688 14.8631Z"
          fill="#212121"
        />
        <path
          d="M25 10.5C25 11.8807 23.8807 13 22.5 13C21.1193 13 20 11.8807 20 10.5C20 9.11929 21.1193 8 22.5 8C23.8807 8 25 9.11929 25 10.5Z"
          fill="#212121"
        />
        <path
          d="M6 1C3.23858 1 1 3.23858 1 6V26C1 28.7614 3.23858 31 6 31H26C28.7614 31 31 28.7614 31 26V6C31 3.23858 28.7614 1 26 1H6ZM3 6C3 4.34315 4.34315 3 6 3H26C27.6569 3 29 4.34315 29 6V26C29 27.6569 27.6569 29 26 29H6C4.34315 29 3 27.6569 3 26V6Z"
          fill="#212121"
        />
      </svg>
    </>
  );
}

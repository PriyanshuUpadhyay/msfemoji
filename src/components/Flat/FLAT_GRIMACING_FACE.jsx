import React from "react";
export function FLAT_GRIMACING_FACE() {
  return (
    <>
      <svg
        width="100%"
        height="100%"
        viewBox="0 0 32 32"
        fill="none"
        xmlns="http://www.w3.org/2000/svg"
      >
        <path
          d="M15.9989 29.9978C25.3333 29.9978 29.9978 23.7303 29.9978 15.9989C29.9978 8.26751 25.3333 2 15.9989 2C6.66443 2 2 8.26751 2 15.9989C2 23.7303 6.66443 29.9978 15.9989 29.9978Z"
          fill="#FFB02E"
        />
        <path
          d="M10.5 16C12.9853 16 15 13.9853 15 11.5C15 9.01472 12.9853 7 10.5 7C8.01472 7 6 9.01472 6 11.5C6 13.9853 8.01472 16 10.5 16Z"
          fill="white"
        />
        <path
          d="M21.5 16C23.9853 16 26 13.9853 26 11.5C26 9.01472 23.9853 7 21.5 7C19.0147 7 17 9.01472 17 11.5C17 13.9853 19.0147 16 21.5 16Z"
          fill="white"
        />
        <path
          d="M12 14C13.1046 14 14 13.1046 14 12C14 10.8954 13.1046 10 12 10C10.8954 10 10 10.8954 10 12C10 13.1046 10.8954 14 12 14Z"
          fill="#402A32"
        />
        <path
          d="M20 14C21.1046 14 22 13.1046 22 12C22 10.8954 21.1046 10 20 10C18.8954 10 18 10.8954 18 12C18 13.1046 18.8954 14 20 14Z"
          fill="#402A32"
        />
        <path
          d="M8 22C8 21.1089 8.38851 20.3085 9.00537 19.759L22.984 19.7496C23.607 20.2993 24 21.1038 24 22C24 22.8941 23.6089 23.6968 22.9884 24.2464L9.01571 24.25C8.39285 23.7003 8 22.896 8 22Z"
          fill="#BB1D80"
        />
        <path
          d="M23 20.5V19.7639C22.4692 19.2889 21.7684 19 21 19H11C10.2316 19 9.53076 19.2889 9 19.7639V20.5C9 21.0523 9.44772 21.5 10 21.5H22C22.5523 21.5 23 21.0523 23 20.5Z"
          fill="white"
        />
        <path
          d="M21 25C21.7684 25 22.4692 24.7111 23 24.2361V23.5C23 22.9477 22.5523 22.5 22 22.5H10C9.44772 22.5 9 22.9477 9 23.5V24.2361C9.53076 24.7111 10.2316 25 11 25H21Z"
          fill="white"
        />
      </svg>
    </>
  );
}
